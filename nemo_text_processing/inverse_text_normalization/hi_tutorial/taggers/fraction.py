...
class FractionFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        #graph_fraction |= pynini.string_file(get_abs_path("data/fractions/fraction_exceptions.tsv")).invert()
        self.fractional = ((pynini.closure(NEMO_SIGMA) + graph_fraction) @ graph_cardinal).optimize()
 
        integer = pynutil.insert("integer_part: \"") + graph_cardinal + pynutil.insert("\" ")
        integer += delete_space
        integer += pynutil.delete("बटा")  # used to demarcate integer and fractional parts
        ...
        graph_fraction = pynini.closure(graph_fraction + delete_space) + graph_fraction
        self.graph = graph_fraction
        forward_slash = pynutil.delete("बटा")
        graph = pynini.closure(integer + delete_space, 0, 1) + graph_fraction
